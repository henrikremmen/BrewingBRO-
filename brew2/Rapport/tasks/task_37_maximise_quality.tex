...
\end{equation}
or equivalently $\min -q$ in minimisation form.

\subsection{Quality Constraints}
\label{sec:task7-constraints}
The volume, colour, and process constraints remain active. For the 5000 L amber problem, the required totals are fixed constants:
\begin{equation}
  \Mtot = 763.158~\kg,\qquad
  H_{\mathrm{req}} = 6.842~\kg,\qquad
  Y_{\mathrm{req}} = 3.750~\kg.
\end{equation}
The three group-quality constraints are therefore linear when rewritten with $q$ as the lower bound:
\begin{align}
  ...
  &\geq qH_{\mathrm{req}}, \label{eq:q-hop}\\
  \sum_{i \in Y} Q^Y_i y_i
  &\geq qY_{\mathrm{req}}. \label{eq:q-yeast}
\end{align}
The terms $q\Mtot$, $qH_{\mathrm{req}}$, and $qY_{\mathrm{req}}$ are products of $q$ with constants, not products of decision variables. The bound $0\leq q\leq5$ is added. The model remains a MILP because $q$ is continuous and the only integer variables are still the two process variables handled by the same brute-force enumeration as in Section~\ref{sec:relaxation-bruteforce}.
