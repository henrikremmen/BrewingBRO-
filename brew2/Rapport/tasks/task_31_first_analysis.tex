...
The first cost analysis uses the cheapest available barley, malt, malt extract, hops, and yeast from the dataset. With $V$ denoting final beer volume in litres, the separate-route cost functions are
\begin{align}
...
Table~\ref{tab:task31-cost-breakdown} shows how these costs are split between fermentable sources, hops, yeast, and process costs for three representative volumes. Hops and yeast increase almost linearly with volume in all routes; the important route changes come from the trade-off between fixed process costs and fermentable-source variable costs.

\input{generated_tables/task31_cost_breakdown}

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{bilder/task_31_cost_volume.png}
\caption{Task 1 cost as a function of beer volume for the barley, malt, malt extract, and combined minimum-cost routes.}
